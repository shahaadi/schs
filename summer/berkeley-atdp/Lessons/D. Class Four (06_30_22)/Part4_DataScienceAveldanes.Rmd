...
```{r}
sample_sd <- function(vector_values){
  return(sqrt(sum((vector_values - mean(vector_values)) ^ 2 / (length(vector_values) - 1))))
}

sample_sd(c(2, 3, 56, 12, 32)) == sd(c(2, 3, 56, 12, 32))
```
...
simulation <- function(n_points){
  ...
  ## determine if points lie in the circle 
  distance <-x^2 +y^2
...
```{r}
simulation_plot <- function(n_points){
  ## sample points 
  x <- runif(n_points, min = -1, max =1)
  y <- runif(n_points, min =-1, max =1)
  ## determine if points lie in the circle 
  ...
  plot(x,y, col = within_circle +1, pch =20,
      main = paste0("Pi estimate is", pi_estimate))
}
simulation_plot(1000000)
```
